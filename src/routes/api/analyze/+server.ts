...
【厳守すべき4つのルール】
1. **情報優先度と専門用語の強制**: 講義ノート（スライド等の資料テキスト）を構成の主軸とし、ノートにある主要な見出しや専門用語を ”必ず3つ以上” ピックアップして感想文の軸に据えてください（例：「今回の講義では、特に『〇〇』や『〇〇』について深く学びました」）。文字数が足りない場合は雑談で埋めるのではなく、ピックアップした専門用語の解釈を厚くしてください。
2. **文字起こしの賢い抽出**: 音声データ（文字起こし）は、ピックアップした専門用語の周辺（文脈）に関連する「講師独自の解説」「具体的な補足」「テストへの言及」を拾うためだけに使用してください。講義の主題と無関係な雑談（部活、事務連絡等）はノイズとして完全に消去してください。
3. **連続したエッセイ形式への固定**: 箇条書きやセクション分け（【本編】など）を一切敷かず、最初から最後まで流れるような「一つの感想文」として出力してください。
4. **書き言葉へのリライトとラベル**: 文字起こしの口語（えー、あのー等）は排除し、「〜という説明があり、理解が深まりました」「先生が〜とおっしゃっていたのが印象的でした」といった自然な一人称（です・ます調）の書き言葉にリライトしてください。また、文字起こし由来の補足が含まれる文には必ず「🔊 講義内の補足」というラベル等を添えてください。
