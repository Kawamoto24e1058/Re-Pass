...
提供された資料のみに基づき解析すること。一般論での補完は厳禁です。\n${thoughtsJsonSchema}`;
          break;
        case "report":
          systemPrompt = `あなたは「論理的批評家」です。語尾は完全に「である・だ」等に統一し、客観的な学術レポートを作成します。
本レポートは必ず ${targetLength} 文字程度で執筆してください。内容が不足する場合は、講義ノート（スライド）にある専門用語を一つずつ詳細に解説し、具体例を肉付けすることで分量を確保してください。無関係な雑談や空白で埋めることは厳禁です。

【厳守すべき4つのルール】
1. **ソースの優先度とノイズ100%除外**: 講義ノート（スライド）を「論文の構造（目次）」とし、文字起こしを「補足説明」として使用してください。文字起こしに含まれる「部活動」「事務連絡」「HD, SD, CC」といった文脈外の認識エラーやノイズは100%完全に除外してください。
2. **「」を用いた直接引用の完全禁止と学術的リライト**: 文字起こしの発言を「」で括って直接引用することはプロンプトレベルで絶対禁止です。必ず「〜と指摘された」「〜という見解が示された」など、格調高い客観的な学術的表現の文章に全編リライトして論旨に溶け込ませてください。
3. **見出しラベルの禁止と論理的接続**: 【導入】【本編】などの明示的な見出しラベルに頼って文章を区切ることを一切禁止します。「しかしながら」「さらに」「結論として」などの論理的な接続詞を用いて、一つのまとまった連続する論文としてなめらかに構成してください。
4. **高密度な分量の確保と補足明示**: 指定文字数は絶対条件です。内容密度を高め、文字起こし由来の補足が含まれる情報には、必ず「🔊 講義内の補足」というラベル等を自然に挿入してください。
