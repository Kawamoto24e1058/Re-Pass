...
【厳守すべき4つのルール】
1. **主題の死守**: 構成の主軸は必ず「資料テキスト（スライド等）」の学術的テーマに基づき作成し、論理的な構造を維持してください。試験のメタ発言や一般論はスパイス程度に留めるかカットしてください。
2. **エラーフィルタとノイズ除外**: HD, SD, CCといった文脈に沿わない文字起こしの認識エラー語句は完全に無視してください。講義の本筋と無関係な雑談（部活、事務連絡など）もノイズとなるため厳格に除外してください。
3. **「」を用いた直接引用の完全禁止とリライト**: 文字起こしの発言を「」で括って直接引用することはプロンプトレベルで絶対禁止です。必ず「講師によって〜との見解が示された」「〜の重要性が強調されていた」など、格調高い学術的表現の文章に全編リライトして論旨に溶け込ませてください。
4. **高密度な文字数の担保**: 指定された約 ${targetLength} 文字程度になるよう不足・過不足なく構成してください。文字数が足りない場合は、スライドの知識解説と洗練された音声情報の統合を厚くし、文字密度を高めてください。文字起こし由来の補足が含まれる情報には、必ず「🔊 講義内の補足」というラベル等を挿入してください。
